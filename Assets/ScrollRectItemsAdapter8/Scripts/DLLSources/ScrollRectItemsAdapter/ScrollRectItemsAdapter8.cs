...
namespace frame8.Logic.Misc.Visual.UI.ScrollRectItemsAdapter
{
	...
	where TItemViewsHolder : BaseItemViewsHolder//, new()
	{
		...
		public int RecyclableItemsCount { get { return _InternalState.recyclableItemsCount; } }
			...
			Func<float, bool> onProgress = null)
		{
			...
			float initialInsetFromParent = -1f, targetInsetFromParent = -1f; // setting a value because of compiler, but it's initialized at least once in the loop below
			bool needToCalculateInitialAndTargetInsets = true;
			...
			do
			{
				...
				if (needToCalculateInitialAndTargetInsets)
				{
					initialInsetFromParent = _Params.content.GetInsetFromParentEdge(_Params.viewport, _InternalState.startEdge);
					targetInsetFromParent = ClampContentStartOffsetFromViewportStart(minContentOffsetFromVPAllowed, itemIndex, normalizedOffsetFromViewportStart, normalizedPositionOfItemPivotToUse);
					if (!_Params.loopItems)
						needToCalculateInitialAndTargetInsets = false; // always updating, because the scrollview can loop at any time
				}
		...
		{
			float tenIfLooping = _Params.loopItems ? 10f : 0f; // if looping, there's no need to clamp. in addition, clamping would cancel a scrollTo if the content is exactly at start or end
			float maxContentOffsetFromVPAllowed = tenIfLooping + 0f;
			minContentOffsetFromVPAllowed -= tenIfLooping;
